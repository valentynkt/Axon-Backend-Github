...
/// </summary>
[TestFixture]
public class IdentityResolutionAlgorithmTests : IdentityResolutionTestBase
{
    ...
    public async Task Test_RESOLVE_credential_first_ExistingCredential_ShouldReturnPrincipal()
    {
        ...
        await MockPrincipalRepository.Received(1).FindByCredentialAsync(
            ...
            TestDataFixtures.DynamicIssuer,
    ...
    public async Task Test_RESOLVE_wallet_verified_wins_MultipleVerifiedWallets_ShouldReturnOwner()
    {
        ...
        {
            { (TestDataFixtures.SolanaMainnetChain, address1), ownedWallet.Id },
            { (TestDataFixtures.SolanaMainnetChain, address2), unownedWallet.Id }
        });
    ...
    public async Task Test_RESOLVE_ambiguity_tie_break_SameAuthority_ShouldUseEarliestPrincipal()
    {
        ...
        var ownershipA = TestDataFixtures.CreatePendingSigningOwnership(principalA.Id, wallet.Id);
        var ownershipB = TestDataFixtures.CreatePendingSigningOwnership(principalB.Id, wallet.Id);
    ...
    public async Task Integration_CompleteResolutionFlow_ShouldFollowCorrectPriority()
    {
        ...
        var principalWithCredential = TestDataFixtures.CreatePrincipalA(); // Has credential
        var principalWithWallet = AxonPrincipal.CreateHuman(); // No credential, has wallet
        var wallet = TestDataFixtures.CreateW1Main();

        // Setup wallet ownership for second principal
        var ownership = TestDataFixtures.CreateVerifiedSigningOwnership(principalWithWallet.Id, wallet.Id);
