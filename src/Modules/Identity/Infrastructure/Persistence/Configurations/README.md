...
- **`AxonPrincipalConfiguration.cs`** - Primary aggregate configuration with chain defaults mapping
- **`WalletConfiguration.cs`** - Wallet aggregate with unique chain-address constraint
- **`CredentialConfiguration.cs`** - Identity credential entity with unique provider-issuer-subject constraint
- **`WalletOwnershipConfiguration.cs`** - Ownership relationship with verified signing owner constraint
- **`PrincipalChainDefaultConfiguration.cs`** - Chain default wallet preferences per principal
